/*
 * JOGADOR-CLIENTE (jogador_cliente.c)
 * Este é o Convidado do jogo.
 *
...
int minha_vez = 2; // Cliente começa em segundo
...
void *receive_thread_cliente(void *arg) {
    ...
    while ((nBytes = recv(servidor_sock, buffer_resposta, sizeof(buffer_resposta), 0)) > 0) {
...
int main() {
    ...
    char input_buffer[256]; // Buffer de I/O
    char ip_servidor[16];
    ...
    if ((servidor_sock = socket(AF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) {
        printf("Nao foi possivel criar socket: %d\n", WSAGetLastError());
        return 1;
    }

    // 3. Pergunta pelo IP (com fgets)
    printf("Digite o IP do Jogador-Servidor (Anfitriao) (ex: 127.0.0.1): ");
    if (fgets(input_buffer, sizeof(input_buffer), stdin) == NULL) return 1;
    remover_newline(input_buffer);
    strncpy(ip_servidor, input_buffer, 15);
    ip_servidor[15] = '\0'; // Garante terminação

    // 4. Prepara a estrutura sockaddr_in
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = inet_addr(ip_servidor);
    server_addr.sin_port = htons(8888);

    // 5. Inicializa os tabuleiros locais
    ...
    // 6. Chama o posicionamento de navios
    posicionar_navios(meu_jogador.tabuleiro_defesa);

    // 7. Conecta ao Servidor
    printf("Conectando ao Anfitriao no IP %s...\n", ip_servidor);
    if (connect(servidor_sock, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
        printf("Conexao falhou: %d\n", WSAGetLastError());
        return 1;
    }

    printf("Conectado ao Jogador-Servidor! O jogo vai comecar.\n");
    
    // 8. Cria a Thread de Recebimento
    if (pthread_create(&recv_thread, NULL, receive_thread_cliente, NULL) < 0) {
        perror("Nao foi possivel criar a thread de recebimento");
        return 1;
    }

    // 9. Loop Principal (Thread de Envio)
    printf("Aguardando o oponente (Servidor) jogar...\n");
    while (fgets(input_buffer, sizeof(input_buffer), stdin) != NULL) {
        remover_newline(input_buffer); // Remove o \n

        if (minha_vez == 1) { 
            if (strlen(input_buffer) > 0) {
                if (send(servidor_sock, input_buffer, strlen(input_buffer), 0) < 0) {
                    printf("Envio falhou.\n");
                    break;
                }
                minha_vez = 2;
